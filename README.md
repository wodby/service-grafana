...
| Service links | Grafana Loki (`loki`), optional |
...
Use this service through [Grafana application stack](https://github.com/wodby/stack-grafana), [Grafana Loki application stack](https://github.com/wodby/stack-loki), or reference
`grafana` from a custom Wodby stack.
...
The service exposes an optional `loki` link and a neutral `datasources` config.
Stacks can override that config to provision linked data sources without
changing the standalone Grafana experience. The Grafana Loki stack uses this
contract to add its Loki service as Grafana's default data source.
